...
export function initializeBinaryStreams(app: StreamApplication): void {
  console.log('[signalk:streams] initializeBinaryStreams called, app.server exists:', !!app.server)
  if (!app.server) {
    debug('HTTP server not available, skipping binary stream initialization')
    console.log('[signalk:streams] HTTP server not available, skipping binary stream initialization')
    return
  }
  ...
  app.server.on('upgrade', (request: IncomingMessage, socket: any, head: Buffer) => {
    console.log(`[signalk:streams] Upgrade request received: ${request.url}, headers.host: ${request.headers.host}`)
    try {
      ...
      if (streamMatch) {
        ...
        console.log(`[signalk:streams] Matched radar stream pattern: ${streamId}`)
      } else {
        // Not a binary stream endpoint, let other handlers process
        console.log(`[signalk:streams] No match for path, ignoring: ${pathname}`)
        return
      }

      debug(`WebSocket upgrade request for stream: ${streamId}`)
      console.log(`[signalk:streams] Processing WebSocket upgrade for stream: ${streamId}`)
      ...
      if (app.securityStrategy && typeof app.securityStrategy.shouldAllowWrite === 'function') {
        try {
          ...
          if (app.securityStrategy.authorizeWS) {
            ...
            principal = (request as any).skPrincipal || { identifier: 'unknown' }
          }
        } catch (error) {
      ...
      console.log(`[signalk:streams] Creating WebSocket server for stream: ${streamId}`)
      const wss = new WebSocket.Server({ noServer: true })
      wss.handleUpgrade(request, socket, head, (ws) => {
        debug(`WebSocket connected to stream: ${streamId}`)
        console.log(`[signalk:streams] WebSocket connected to stream: ${streamId}`)
